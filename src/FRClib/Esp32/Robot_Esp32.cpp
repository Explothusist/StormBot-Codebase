...
namespace frclib {
    ...
    TimedRobot::TimedRobot(int autonomous_length):
        ...
        m_start_of_auto{ } // Some value here, perhaps? But how?
    ...
    void TimedRobot::pollState() { // FANCY LOOP
        if (m_first_auto_trigger) {
            ...
            if (triggered) {
                ...
                m_start_of_auto = std::chrono::system_clock::now();
            }
        }else if (m_state == Autonomous) {
            // int now = vex::timer::system();
            std::chrono::time_point<std::chrono::_V2::system_clock, std::chrono::duration<long long, std::nano>> now = std::chrono::system_clock::now();
            std::chrono::duration<double> elapsed = now - m_start_of_auto;

            if (elapsed.count() > (m_autonomous_length*1000)) {
                m_state = Teleop;
            }
        }
    ...
    void TimedRobot::robotInternal() { // bool is_original
        while (true) {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(m_frame_delay)); // Update to correct
